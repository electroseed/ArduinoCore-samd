...
class SPIClass {
  public:
	  SPIClass(SERCOM *p_sercom, uint8_t uc_pinMISO, uint8_t uc_pinSCK, uint8_t uc_pinMOSI, SercomSpiTXPad, SercomRXPad, EPioType ul_PinType = PIO_NOT_A_PIN);
  ...
  void begin();
	void begin(uint8_t miso_pin,uint8_t mosi_pin,uint8_t sck_pin)
	{
//		setPins(miso_pin,mosi_pin,sck_pin);
		begin();
	}
  void end();
