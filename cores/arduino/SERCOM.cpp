...
void SERCOM::initSPIMode(SercomSpiClockMode clockMode)
{
...
uint8_t SERCOM::SPIFlags()
{
	return sercom->SPI.INTFLAG.reg;
}

uint8_t SERCOM::SPIStatus()
{
	return sercom->SPI.STATUS.reg;
}

void SERCOM::initSPIClock(SercomSpiClockMode clockMode,uint32_t baudrate)
{
  initSPIMode(clockMode);
...
{
	sercom->SPI.CTRLA.bit.ENABLE = 1;			// Setting the enable bit to 1
	while (sercom->SPI.SYNCBUSY.bit.ENABLE);	// Waiting then enable bit from SYNCBUSY is equal to 0;
	sercom->SPI.CTRLB.bit.RXEN = 1;				// Enable Receiver, this is done here due to errata issue
...
{
	while (sercom->SPI.SYNCBUSY.bit.ENABLE);	// Waiting then enable bit from SYNCBUSY is equal to 0;
	sercom->SPI.CTRLA.bit.ENABLE = 0;			// Setting the enable bit to 0
}
...
void SERCOM::writeSPIIntReg(uint8_t data)
{
	sercom->SPI.INTFLAG.reg = data;
}
...
bool SERCOM::isBufferOverflowErrorSPI()			// Clears flag as well
{
  bool bo = sercom->SPI.STATUS.bit.BUFOVF;
...
bool SERCOM::startTransmissionWIRE(uint8_t address, SercomWireReadWriteFlag flag)
{
  ...
  uint32_t end_time = micros() + timeOutMicros;
  // Address Transmitted
  if ( flag == WIRE_WRITE_FLAG ) // Write mode
  {
    while( (!sercom->I2CM.INTFLAG.bit.MB) && micros() < end_time  )
    {
  ...
  {
    while( (!sercom->I2CM.INTFLAG.bit.SB) && micros() < end_time  )
    {
...
bool SERCOM::sendDataMasterWIRE(uint8_t data)
{
  ...
  uint32_t end_time = micros() + timeOutMicros;
  while(!sercom->I2CM.INTFLAG.bit.MB && micros() && micros() < end_time)
  {
...
uint8_t SERCOM::readDataWIRE( void )
{
  ...
  {
  	uint32_t end_time = micros() + timeOutMicros;
    while( sercom->I2CM.INTFLAG.bit.SB == 0 && micros() < end_time )
    {
...
static const struct {
  ...
  uint8_t clk_bit;
	...
	SERCOM0,GCM_SERCOM0_CORE,SERCOM0_IRQn,PM_APBCMASK_SERCOM0_Pos,
	SERCOM1,GCM_SERCOM1_CORE,SERCOM1_IRQn,PM_APBCMASK_SERCOM1_Pos,
	SERCOM2,GCM_SERCOM2_CORE,SERCOM2_IRQn,PM_APBCMASK_SERCOM2_Pos,
	SERCOM3,GCM_SERCOM3_CORE,SERCOM3_IRQn,PM_APBCMASK_SERCOM3_Pos,
#if defined(SERCOM4)
	SERCOM4, GCM_SERCOM4_CORE, SERCOM4_IRQn,PM_APBCMASK_SERCOM4_Pos,
#endif
#if defined(SERCOM5)
	SERCOM5, GCM_SERCOM5_CORE, SERCOM5_IRQn,PM_APBCMASK_SERCOM5_Pos,
#endif
};
...
//  uint8_t clk_bit = sercomData[idx].clk_bit;
//  PM->APBCMASK.reg  = 1<<clk_bit;					// Crashes, but it's done in wiring / init()
  // Setting NVIC
